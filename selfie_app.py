...
def detect_peace_sign(hand_landmarks):
    ...
    try:
        ...
        # *** IMPROVEMENT FIX ***
        # dynamic threshold: depends on hand size (distance between index and pinky knuckles)
        idx_mcp_x = hand_landmarks.landmark[5].x  # Index finger knuckle
        pinky_mcp_x = hand_landmarks.landmark[17].x # Pinky finger knuckle
        hand_width_est = abs(idx_mcp_x - pinky_mcp_x) + 1e-6
        # *** END IMPROVEMENT FIX ***

        # require separation at least ~0.2 * hand_width_est (empirical)
        separated = sep > (0.2 * hand_width_est)
...
def main():
    ...
    try:
        while True:
            # *** ERROR FIX: 1. Read to 'frame_original' ***
            ret, frame_original = cap.read()
            if not ret:
                print('Failed to grab frame')
                break

            # *** ERROR FIX: 2. Create flipped 'frame' for display only ***
